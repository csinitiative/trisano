...
i = 0
MorbidityEvent.find_in_batches(:include => [ :interested_party => { :person_entity => :addresses } ], :conditions => "participations.id IS NOT NULL", :batch_size => 500 ) do |event_group|
  i += 1
...
ContactEvent.find_in_batches(:include => [ :interested_party => { :person_entity => :addresses } ], :conditions => "participations.id IS NOT NULL", :batch_size => 500 ) do |event_group|
  i += 1
  puts "    * Processing #{i.ordinalize} group of 500 human events"
  event_group.each do | event |
    unless event.try(:address)
