...
if RUBY_PLATFORM =~ /java/
  ...
  namespace :trisano do
    namespace :metadata do
      ...
      task :setup_constants => :load_jars do
        AllTablesGroupName = 'All Formbuilder Tables';
        DefaultFieldType = Java::OrgPentahoPmsSchemaConceptTypesFieldtype::FieldTypeSettings::DIMENSION
        MaxX = 800
      ...
      task :load_jars => :clean_metadata do
        require_jars Dir.glob(File.join(server_dir, 'tomcat/webapps/pentaho/WEB-INF/lib', '*.jar'))
        require_jars Dir.glob(File.join(server_dir, 'tomcat/common/lib', 'postgres*.jar'))
      end

      task :clean_metadata do
        old_dir = Dir.pwd
        Dir.chdir File.join(server_dir, 'pentaho-solutions', 'TriSano')
      ...
      task :init_metadata => [:load_jars, :setup_constants, :clean_metadata]

      task :build_metadata => :init_metadata do
        old_dir = Dir.pwd
        Dir.chdir File.join(server_dir, 'pentaho-solutions', 'TriSano')
