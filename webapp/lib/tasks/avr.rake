...
namespace :avr do
    namespace :db do
        ...
        def get_warehouse_connection(superuser = false, options = {})
        ...
        def create_trisano_schema(conn)
            exec_stmts conn, [
                'BEGIN;',
        ...
        def create_population_schema(conn)
            exec_stmts conn, [
                'BEGIN;',
        ...
        def create_plpgsql(conn)
            conn.exec('CREATE OR REPLACE LANGUAGE plpgsql;')
        end
    ...
    $$
        ...
        def create_more_misc_funcs(conn)
            exec_stmts conn, [
...
BUILD_FORM_TABLES
        ...
        def create_schema_comments(conn)
            exec_stmts conn, [
                'BEGIN;',
        ...
        def create_swap_schema_function(conn)
            conn.exec('BEGIN;')
...
SWAP_SCHEMAS
        ...
        def create_population_tables(conn)
            exec_stmts conn, [
                'BEGIN',
...
#         CROSS JOIN (SELECT DISTINCT race FROM trisano.dw_patient_races_view) b;
        ...
        task :init => :create_warehouse_db do
            get_warehouse_connection(true) do |conn|
                create_trisano_schema conn
                create_population_schema conn
                create_plpgsql conn
                create_hstore conn
                create_date_funcs conn
                create_misc_funcs conn
                create_mondrian_aggs conn
                create_more_misc_funcs conn
                create_form_function conn
                create_schema_comments conn
                create_swap_schema_function conn
                create_population_tables conn
            end
        end
    end
end
