...
describe "/events/_places_search.html.haml" do
  ...
  before(:each) do
    ...
    @canonical_address = mock_model(Address)
    @place_types.stub!(:code_description).and_return("Warm Wading Pool")
    @county.stub!(:code_description).and_return("Beaver")
    @place_entity.stub!(:canonical_address).and_return(@canonical_address)
    @canonical_address.stub!(:street_number).and_return("123")
    @canonical_address.stub!(:street_name).and_return("West 23 East 99 North")
    @canonical_address.stub!(:city).and_return("O-town")
    @canonical_address.stub!(:county).and_return(@county)
    @canonical_address.stub!(:postal_code).and_return("99999")
