...
describe 'form builder patient-level core field configs for contact events' do
  ...
  [{:name => 'Contact last name', :tab_name => DEMOGRAPHICS},
    ...
    it "should support before and after on the '#{test[:name]}' field" do
      ...
      add_question_to_before_core_field_config(@browser, test[:name], {:question_text => before_question, :data_type => "Single line text", :help_text => before_help_text, :short_name => get_random_word})
      add_question_to_after_core_field_config(@browser, test[:name], {:question_text => after_question, :data_type => "Single line text", :help_text => after_help_text, :short_name => get_random_word})
