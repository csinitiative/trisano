...
class CdcExport < ActiveRecord::Base
  ...
  class << self
    ...
    def cdc_deletes(start_mmwr, end_mmwr)
      where = [ "sent_to_cdc=true AND ((deleted_at BETWEEN #{sanitize_sql_for_conditions(["'%s'", start_mmwr.mmwr_week_range.start_date]).untaint} AND #{sanitize_sql_for_conditions(["'%s'", end_mmwr.mmwr_week_range.end_date]).untaint})" ]
    ...
    def verification_records(mmwr_year, mmwr_week=nil)
      ...
      where = '"MMWR_year"=' + sanitize_sql_for_conditions(["%d", mmwr_year.to_s]).untaint + ' AND deleted_at IS NULL'
