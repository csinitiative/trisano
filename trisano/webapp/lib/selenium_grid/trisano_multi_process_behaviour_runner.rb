...
class TriSanoMultiProcessSpecRunner
  ...
  def initialize(max_concurrent_processes = 10, reports_prefix = "Default")
    ...
    puts "Initializing TriSanoMultiProcessSpecRunner with #{@max_concurrent_processes.to_s} max concurrent processes and reports prefix #{@reports_prefix}"
  end
  ...
  def compile_report
    ...
    FileUtils.rm("#{screenshot_dir}/*.html")
