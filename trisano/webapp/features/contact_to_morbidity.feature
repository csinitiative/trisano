Feature: Promoting Contact Events to Morbidity Events
  ...
  Scenario: Visiting contact show mode has 'promote' link
    Given a morbidity event for last name Smith with disease Anthrax in jurisdiction Davis County
  ...
  Scenario: Promoted morbidity event displays parent and disease forms
    Given a published disease form called "MA1" for morbidity events with Anthrax
    And a published disease form called "CA1" for contact events with Anthrax
    And a morbidity event for last name Smith with disease Anthrax in jurisdiction Davis County
    And there is a contact named Jones
    And I am logged in as a super user
