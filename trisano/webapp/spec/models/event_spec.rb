...
#  }
  ...
  describe 'converted to a cdc export record' do
    fixtures :diseases
    ...
    it "should display an unknown county code as 999" do
      with_event do |event|
        event.to_cdc[27..29].should == '999'
      end
    end

    it "should display birthday as YYYYMMDD" do
      with_event do |event|
        event.to_cdc[30..37].should == '19750101'
      end
    end
    ...
    it "should display an unknown birthday as 99999999" do
      @event_hash['active_patient']['person']['birth_date'] = nil      
      with_event do |event|
        event.to_cdc[30..37].should == '99999999'
      end
    end

    it "should display age at onset as a 3 digit field" do
      with_event do |event|
        event.to_cdc[38..40].should == event.age_at_onset.to_s.rjust(3, '0')
      end
    end

    it "should display age type as 1 digit field" do
      with_event do |event|
        event.to_cdc[41...42].should == "0"
      end
    end
    ...
    it "should display event date a YYMMDD" do
      with_event do |event|
        event.to_cdc[45..50].should == Date.today.strftime("%y%m%d")
      end
    end

    it "should display which event date was used as a one digit code" do
      with_event do |event|
        event.to_cdc[51...52].should == "1"
      end
    end

    it "should display case status as a one digit code"
