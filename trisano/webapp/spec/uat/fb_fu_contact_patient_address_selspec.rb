...
describe 'form builder patient-level address core field configs for contacts' do
  ...
  fields = [{:name => 'Contact street number', :label => 'contact_event_active_patient__active_primary_entity__address_street_number', :tab_name => DEMOGRAPHICS, :fu_value => '444', :no_fu_value => '222'},
    {:name => 'Contact street name', :label => 'contact_event_active_patient__active_primary_entity__address_street_name', :tab_name => DEMOGRAPHICS, :fu_value => 'Chaff Drive', :no_fu_value => 'Chart Drive'},
    {:name => 'Contact unit number', :label => 'contact_event_active_patient__active_primary_entity__address_unit_number', :tab_name => DEMOGRAPHICS, :fu_value => '444', :no_fu_value => '222'},
    {:name => 'Contact city', :label => 'contact_event_active_patient__active_primary_entity__address_city', :tab_name => DEMOGRAPHICS, :fu_value => 'Brigham City', :no_fu_value => 'Provo'},
    {:name => 'Contact state', :label => 'contact_event_active_patient__active_primary_entity__address_state_id', :tab_name => DEMOGRAPHICS, :fu_value => 'Utah', :no_fu_value => 'Texas'},
    {:name => 'Contact county', :label => 'contact_event_active_patient__active_primary_entity__address_county_id', :tab_name => DEMOGRAPHICS, :fu_value => 'Utah', :no_fu_value => 'Davis'},
    {:name => 'Contact zip code', :label => 'contact_event_active_patient__active_primary_entity__address_postal_code', :tab_name => DEMOGRAPHICS, :fu_value => '89011', :no_fu_value => '80001'}
  ]
  ...
  data_types = [{:name => 'Single line text', :values => nil, :answer => get_unique_name(5), :entry_type => "type"},
    ...
    {:name => 'Radio buttons', :values => ["Yes","No","Maybe"], :answer => "3", :entry_type => "radio"},# answer 3 = "Maybe"
    {:name => 'Checkboxes', :values => ["Pink","Red","Purple"], :answer => "3", :entry_type => "check"},
  ...
  fields.each do |test|  
    ...
    it "should create a contact follow-up container" do
      ...
      add_core_follow_up_to_view(@browser, "Default View", test[:fu_value], test[:name]).should be_true
    end
    ...
    it "should add all the followup questions when #{test[:name]} is assigned the value #{test[:fu_value]}" do  
      ...
      set_fields(@browser, {test[:label] => test[:fu_value]})
    ...
    it "should remove all the investigator questions when the value is changed to #{test[:no_fu_value]}" do         
      set_fields(@browser, {test[:label] => test[:no_fu_value]})
    ...
    data_types.each do |data_type|
      ...
      it "should save values for #{follow_up_question}" do 
        ...
        when 'type'
          ...
          @browser.is_text_present(data_type[:answer]).should be_true
        when 'check'
          answer_check_investigator_question(@browser, follow_up_question, data_type[:answer]).should be_true
          save_contact_event(@browser).should be_true
          @browser.is_text_present(data_type[:answer]).should be_true
        when 'radio'
          answer_radio_investigator_question(@browser, follow_up_question, data_type[:answer]).should be_true
          save_contact_event(@browser).should be_true
          @browser.is_text_present(data_type[:answer]).should be_true
        end       
      end
