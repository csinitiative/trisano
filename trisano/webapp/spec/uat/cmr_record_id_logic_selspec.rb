...
describe 'System functionality for setting the record ID of a CMR' do
  ...
  it 'should create two CMRs in a row with sequential record numbers that start with the current year' do
    ...
    @browser.type('morbidity_event_active_patient__person_last_name', 'Record')
    @browser.type('morbidity_event_active_patient__person_first_name', 'Number')
    save_cmr(@browser).should be_true
    recNum = get_record_number(@browser)
    click_nav_new_cmr(@browser).should be_true
    @browser.type('morbidity_event_active_patient__person_last_name', 'Next')
    @browser.type('morbidity_event_active_patient__person_first_name', 'Record')
    save_cmr(@browser).should be_true
