...
describe DashboardHelper, "#task_filter_description" do
  ...
  describe 'when no filters applied' do
    it 'should show no filters applied' do
      task_filter_description({}).should == 'Showing all tasks.'
    end
  end
  describe 'when look ahead filter applied' do
    it 'should show the number of days looking ahead' do
      task_filter_description(:look_ahead => '3').should == "Showing all tasks through #{Date.today + 3}."
    end

    it 'should use \'tomorrow\' if looking ahead one day' do
      task_filter_description(:look_ahead => '1').should == 'Showing all tasks through tomorrow.'
    end
  end

  describe 'when look back filter applied' do
    it 'should show the number of days looking back' do
      task_filter_description(:look_back => '3').should == "Showing all tasks starting from #{Date.today - 3}."
    end

    it 'should use \'yesterday\' if looking back one day' do
      task_filter_description(:look_back => '1').should == 'Showing all tasks starting from yesterday.'
    end
  end

  describe 'when look ahead and look back filters applied' do
    it 'should combine descriptions' do
      task_filter_description(:look_back => '1', :look_ahead => '1').should == 'Showing all tasks starting from yesterday through tomorrow.'
    end
  end

  describe 'when look ahead and look back are both zero' do
    it 'should just show today' do
      task_filter_description(:look_back => '0', :look_ahead => '0').should == 'Showing all tasks for today.'
    end
  end
end
