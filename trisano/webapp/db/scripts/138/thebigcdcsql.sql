...
SELECT  DISTINCT ON (b.exp_caseid)
...
--  , SUBSTR(EXTRACT(YEAR from CURRENT_DATE)::text, 3,2) AS exp_year
  , SUBSTR(events."MMWR_year"::text, 3,2) AS exp_year
  ...
  , COALESCE(valgender.value_to, '9', valgender.value_to) AS exp_sex
  , COALESCE(valrace.value_to, '9', valrace.value_to) AS exp_race  
  , COALESCE(valethnicity.value_to, '9', valethnicity.value_to)  AS exp_ethnicity 
-- have to do eventdate and datetype in separate query
...
--  , 'X' as exp_datetype
  ...
  , COALESCE(valoutbreak.value_to, '9', valoutbreak.value_to) AS exp_outbreak
...
FROM 
    ...
    LEFT OUTER JOIN entities_locations
      ON  entities_locations.entity_id = entities.id AND entities_locations.location_type_id = (SELECT id FROM codes WHERE the_code = 'ALT')
...
--      ON  ageType.id = people.age_type_id
    ...
    LEFT  OUTER JOIN external_codes outbreak
      ON  outbreak.id = events.outbreak_associated_id
