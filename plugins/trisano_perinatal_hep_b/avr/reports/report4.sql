...
SELECT
    ...
    COUNT(CASE WHEN contact_type = 'infant' AND (get_contact_hbsag_before(dce.id, hepb_dose1_date)).lab_test_date <= birth_date + INTERVAL '12 months' AND (get_contact_antihb_after(dce.id, hepb_dose1_date)).lab_test_date <= birth_date + INTERVAL '12 months' THEN 1 ELSE NULL END) AS f40,
    COUNT(CASE WHEN contact_type = 'infant' AND (get_contact_hbsag_before(dce.id, hepb_dose1_date)).lab_test_date <= birth_date + INTERVAL '15 months' AND (get_contact_antihb_after(dce.id, hepb_dose1_date)).lab_test_date <= birth_date + INTERVAL '15 months' THEN 1 ELSE NULL END) AS f41,
    COUNT(CASE WHEN contact_type = 'infant' AND ((get_contact_hbsag_after(dce.id, hepb_dose1_date)).lab_test_date IS NULL OR (get_contact_antihb_after(dce.id, hepb_dose1_date)).lab_test_date IS NULL) THEN 1 ELSE NULL END) AS f42,
    COUNT(CASE WHEN contact_type = 'infant' AND (get_contact_hbsag_after(dce.id, hepb_dose1_date)).test_result ~ 'Positive' THEN 1 ELSE NULL END) AS f43,
    COUNT(CASE WHEN contact_type = 'infant' AND (get_contact_antihb_after(dce.id, hepb_dose1_date)).test_result ~ 'Positive' THEN 1 ELSE NULL END) AS f44,
    ...
    COUNT(CASE WHEN contact_type IN ('Sexual', 'Household') AND ((get_contact_hbsag_before(dce.id, hepb_dose1_date)).test_result IS NOT NULL OR (get_contact_antihb_before(dce.id, hepb_dose1_date)).test_result IS NOT NULL) THEN 1 ELSE NULL END) AS f56,
    COUNT(CASE WHEN contact_type IN ('Sexual', 'Household') AND (get_contact_hbsag_after(dce.id, hepb_dose1_date)).test_result ~ 'Positive' THEN 1 ELSE NULL END) AS f57,
    COUNT(CASE WHEN contact_type IN ('Sexual', 'Household') AND (get_contact_antihb_after(dce.id, hepb_dose1_date)).test_result ~ 'Positive' THEN 1 ELSE NULL END) AS f58,
