...
def parse(args)
  ...
  @opts = OptionParser.new("Usage: #{__FILE__} [options]", 40, ' ') do |opts|
    opts.separator ""
...
def populate_form(form)
  ...
  if !@options.approximate_age_no_birthday.nil?
    form['person_entity[person_attributes][approximate_age_no_birthday]'] = @options.approximate_age_no_birthday
  end
  if !@options.birth_gender.nil?
    found = false
    form.field('person_entity[person_attributes][birth_gender_id]').options.each do |s|
  ...
  if !@options.ethnicity.nil?
    found = false
    form.field('person_entity[person_attributes][ethnicity_id]').options.each do |s|
  ...
  if !@options.race.nil?
    ...
    form.field('person_entity[race_ids][]').options.each do |s|
  ...
  if !@options.primary_language.nil?
    found = false
    form.field('person_entity[person_attributes][primary_language_id]').options.each do |s|
  ...
  if !@options.address_street_number.nil?
    form['person_entity[canonical_address_attributes][street_number]'] = @options.address_street_number
  end
  if !@options.address_street_name.nil?
    form['person_entity[canonical_address_attributes][street_name]'] = @options.address_street_name
  end
  if !@options.address_unit_number.nil?
    form['person_entity[canonical_address_attributes][unit_number]'] = @options.address_unit_number
  end
  if !@options.address_city.nil?
    form['person_entity[canonical_address_attributes][city]'] = @options.address_city
  end
  if !@options.address_state.nil?
    found = false
    form.field('person_entity[canonical_address_attributes][state_id]').options.each do |s|
  ...
  if !@options.address_county.nil?
    found = false
    form.field('person_entity[canonical_address_attributes][county_id]').options.each do |s|
  ...
  if !@options.address_postal_code.nil?
    form['person_entity[canonical_address_attributes][postal_code]'] = @options.address_postal_code
  end
  # This needs to be reworked as its own restful resource
  if !@options.telephone_entity_location_type.nil?
    found = false
    form.field('person_entity[telephones_attributes][0][entity_location_type_id]').options.each do |s|
  ...
  if !@options.telephone_area_code.nil?
    form['person_entity[telephones_attributes][0][area_code]'] = @options.telephone_area_code
  end
  if !@options.telephone_number.nil?
    form['person_entity[telephones_attributes][0][phone_number]'] = @options.telephone_number
  end
  if !@options.telephone_extension.nil?
    form['person_entity[telephones_attributes][0][extension]'] = @options.telephone_extension
  end
  if !@options.telephone_delete.nil?
    form['person_entity[telephones_attributes][0][_delete]'] = true
  end
  if !@options.email_address.nil?
    form['person_entity[email_addresses_attributes][1][email_address]'] = @options.email_address
  end
  if !@options.email_address_delete.nil?
    form['person_entity[email_addresses_attributes][1][_delete]'] = true
  end
